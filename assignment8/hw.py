...
def is_sorted(my_list: list) -> bool:
    return True if my_list == sorted(my_list) else False
...
def rotate_left(my_list: list, k: int) -> list:
    removing_items = my_list[:k]
    for i in removing_items:
        my_list.remove(i)

    my_list.extend(removing_items)
    return my_list
...
def rotate_right(my_list: list, k: int) -> list:
    if len(my_list) == 2:
        ...
        removing_items = my_list[k+1:]
    for i in removing_items:
        my_list.remove(i)
    removing_items.extend(my_list)
...
def find_intersection(list1: list, list2: list) -> list:
    convert_to_set_list1 = set(list1)
    convert_to_set_list2 = set(list2)
    result = list(convert_to_set_list1.intersection(convert_to_set_list2))
    return result
